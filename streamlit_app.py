...
PORTFOLIO_VERSION = "portfolio-gate-1.0-correlation"
...
def manual_control(df):
    ...
    for _, r in out.iterrows():
        ...
        if grade == "A" and np.isfinite(q) and q <= .20:
            g, ok = "A", True
        elif grade in {"A", "B"}:
            g, ok = "B", False
        elif grade == "관찰":
...
def render_primary():
    ...
    with st.expander("① 📡 80종목 전체 자동 스캔 · 전략 동결", expanded=True):
        if df.empty:
            st.info("자동 스캔 결과를 기다리는 중입니다."); return
        version = str(df.get("엔진버전", pd.Series(["legacy"])).iloc[0])
        if version != PRIMARY_VERSION:
            st.warning(f"이전 결과 {version} · {PRIMARY_VERSION} 대기 중"); return
...
def render_confirmation():
    ...
    with st.expander("② 🧪 동일 파라미터 2차 재현·스트레스", expanded=True):
        if df.empty:
            st.info("2차 결과를 기다리는 중입니다."); return
        version = str(df.get("확인엔진", pd.Series(["legacy"])).iloc[0])
        if version != CONFIRM_VERSION:
            st.warning(f"이전 결과 {version} · {CONFIRM_VERSION} 대기 중"); return
...
def render_paper():
    ...
    with st.expander("③ 🧾 동결재검증 전진 모의", expanded=True):
        if df.empty:
            st.info("전진 모의 후보가 아직 없습니다."); return
...
def render_gate():
    ...
    with st.expander("④ 🛡️ 최종 전진증거 게이트", expanded=True):
        if df.empty:
            st.info("최종 게이트 결과를 기다리는 중입니다."); return
        gate = str(df.get("게이트", pd.Series(["legacy"])).iloc[0])
        if gate != GATE_VERSION:
            st.warning(f"이전 게이트 {gate} · {GATE_VERSION} 대기 중"); return
        c1,c2,c3,c4 = st.columns(4)
...
def render_portfolio():
    ...
    with st.expander("⑤ 🧩 포트폴리오 중복위험 게이트", expanded=True):
        if df.empty:
            st.info("포트폴리오 상관 검증 결과를 기다리는 중입니다."); return
        version = str(df.get("게이트", pd.Series(["legacy"])).iloc[0])
        if version != PORTFOLIO_VERSION:
            st.warning(f"이전 결과 {version} · {PORTFOLIO_VERSION} 대기 중"); return
        corr = pd.to_numeric(df.get("최대상관", pd.Series(dtype=float)), errors="coerce")
        high = (df.get("중복위험", pd.Series(dtype=str)) == "⚠️").sum()
        allowed = (df.get("포트폴리오허용", pd.Series(dtype=str)) == "✅").sum()
        c1,c2,c3,c4 = st.columns(4)
        c1.metric("추적", f"{len(df)}개")
        c2.metric("고상관 경고", f"{int(high)}개")
        c3.metric("포트폴리오 허용", f"{int(allowed)}개")
        c4.metric("최대 상관", "-" if corr.dropna().empty else f"{corr.max():.3f}")
        st.caption(f"{version} · 최근 1년 일간수익률 · 상관 0.80 이상을 중복위험 군집으로 표시")
        show = df.copy()
        if "최대상관" in show: show["최대상관"] = pd.to_numeric(show["최대상관"], errors="coerce").apply(num)
        cols = [c for c in ["포트폴리오허용","최종상태","동결검증","종목","시장","전략","상관군집","최대상관","최대상관대상","공통거래일","중복위험","포트폴리오대기조건","데이터오류"] if c in show]
        st.dataframe(show[cols], use_container_width=True, hide_index=True)


st.title("🧠 APEX Autonomous Validation v8.6")
st.caption("80종목 전체검정 → 파라미터 동결 2차 → 전진모의 → 최종 통계 → 포트폴리오 상관중복 게이트")
st.warning("연구·모의투자용입니다. 미래 수익을 보장하지 않으며 실계좌 주문 기능은 없습니다.")
checks = run_self_tests()
if not checks or not all(checks.values()):
    st.error(f"엔진 자가검증 실패: {checks}"); st.stop()
