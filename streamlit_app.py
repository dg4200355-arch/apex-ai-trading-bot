...
def render_primary():
    ...
    with st.expander("① 📡 80종목 전체 자동 스캔 · 전략 동결", expanded=True):
        ...
        st.caption(f"{version} · 파라미터/데이터 기준일 동결 · 원본 OHLCV 무결성 검사")
        show=df.copy()
        for c in ["TEST수익","MDD"]:
...
def render_paper():
    ...
    with st.expander("③ 🧾 동결재검증 전진 모의", expanded=True):
        ...
        st.caption(f"{tracker} · FROZEN_VERIFIED만 최종 승격 가능 · 누락 거래일 자동 재생")
        show=latest.copy()
...
st.caption("80종목 전체검정·파라미터 동결 → 2차 재현/스트레스 → 동결재검증 전진모의 → 최종 통계 게이트")
...
render_primary(); render_confirmation(); render_paper(); render_gate()
...
if not run:
    st.info("자동 파이프라인은 평일마다 80종목 전체를 갱신합니다.")
    st.stop()
